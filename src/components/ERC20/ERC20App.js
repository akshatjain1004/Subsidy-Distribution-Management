...
const ERC20App = () => {
    ...
    return (
        <div>
            ...
            {menu === Menu.Create && <ERC20Create importToken={importToken} token={token} setToken={setToken} onClickCreateSample={onClickCreateSample}/>}
            {menu === Menu.Import && <ERC20Import tokenAddress={tokenAddress} />}
            {menu === Menu.CreateSample && <ERC20CreateSample token={token} importToken={importToken} />}
        </div>
    )
}
